...
# FIELDS =["nid","shape","price","color","carat","clarity","cut","report","polish","symmetry",
#          "depth","table","fluorescence","list_price","girdle","culet",
#          "measurement","url","diamond_image_flag","reportNumber","info","alias","fast"]
# file_exists = os.path.exists(CSV_FILE)
# existing_certificates = set()
...
#             if cert:
#                 existing_certificates.add(cert)

# print(f"Loaded {len(existing_certificates)} existing certificates")
...
#             print("Cloudflare blocked – waiting for token refresh")
#             time.sleep(20)
#             continue
...
#         count=0
#         for d in diamonds:
#             cert_no = d.get("reportNumber")
            
#             if cert_no.lower().startswith('lg'):
#                 cert_no=cert_no.replace('lg','')
#             if cert_no and cert_no in existing_certificates:
#                 continue            
#             writer.writerow({"nid": d.get('nid'),
#             "shape": d.get('shape'),
#             "price": d.get('price'),
#             "color": d.get('color'),
#             "carat": d.get('carat'),
...
#             "reportNumber": cert_no,
#             "info": d.get('info'),
#             "alias":d.get('alias'),
#             "fast": d.get('fast')})
#             if cert_no:
#                 existing_certificates.add(cert_no)
#             count += 1

#         print(f"Page {page} → {count} NEW rows inserted (API returned {len(diamonds)})")
#         page += 1
#         time.sleep(1.0)
#         if not diamonds:
